...
describe('WakeDeviceStep', () => {
  it('shows waiting state while waiting for the device', () => {
    ...
    expect(screen.getByText('Wake the sensor')).toBeInTheDocument();
    expect(screen.getByText('Restart').parentElement?.textContent).toBe(WAKE_INSTRUCTION);
    expect(screen.getByText('Waiting for the sensor to connect…')).toBeInTheDocument();
  });
  ...
  it('shows retry action when timed out', async () => {
    ...
    expect(
      screen.getByText(
        'No reading received. Make sure you pressed the restart button and the sensor connected to Wi-Fi.',
      ),
    ).toBeInTheDocument();
  ...
  it('shows expired prompt when calibration window ended', async () => {
    ...
    expect(
      screen.getByText("The sensor's calibration window has ended after 2 minutes. Restart calibration to try again."),
    ).toBeInTheDocument();
