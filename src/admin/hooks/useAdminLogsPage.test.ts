...
describe('useAdminLogsPage', () => {
  ...
  it('ignores stale responses when the query changes quickly', async () => {
    let resolveFirst: (value: { items: AdminLog[]; totalCount: number }) => void;
    const firstPromise = new Promise<{ items: AdminLog[]; totalCount: number }>((resolve) => {
      resolveFirst = resolve;
    });

    mockFetchAdminLogsPage
    ...
    rerender({ query: { ...baseQuery, serialNumber: 'SN-99' } });
    await waitFor(() => expect(result.current.totalCount).toBe(50));

    resolveFirst!({ items: logs, totalCount: 1 });
    await waitFor(() => expect(result.current.loading).toBe(false));
  ...
  it('clears stale rows when a refetch fails', async () => {
    mockFetchAdminLogsPage.mockResolvedValueOnce({ items: logs, totalCount: 1 });
    ...
    mockFetchAdminLogsPage.mockRejectedValueOnce(new Error('timeout'));
    rerender({ query: { ...baseQuery, serialNumber: 'SN-99' } });
    await waitFor(() => expect(result.current.loading).toBe(false));
  ...
  it('clamps the requested page to the available total pages', async () => {
    mockFetchAdminLogsPage.mockResolvedValue({ items: logs, totalCount: 10 });

    renderHook(() => useAdminLogsPage({ ...baseQuery, page: 3 }));
    await waitFor(() => expect(mockFetchAdminLogsPage).toHaveBeenCalled());

    expect(mockFetchAdminLogsPage).toHaveBeenCalledWith({
      ...baseQuery,
      page: 1,
    });
  });
