-- Align device filters with logs tab: separate serial, owner, and plant selects.

DROP FUNCTION IF EXISTS public.get_admin_devices_page(text, text, boolean, integer, integer);
...
BEGIN
  ...
  EXECUTE format(
    ...
    filtered AS (
      SELECT *
      FROM enriched
      WHERE ($1 IS NULL OR $1 = '' OR "serialNumber" = $1)
        AND (
          $2 IS NULL OR $2 = ''
          OR ($2 = '__unassigned__' AND owner_email IS NULL)
          OR owner_email = $2
        )
        AND (
          $3 IS NULL OR $3 = ''
          OR ($3 = '__unassigned_plant__' AND "plantName" IS NULL)
          OR "plantName" = $3
        )
    ),
    total AS (
...
GRANT EXECUTE ON FUNCTION public.get_admin_devices_page(text, text, text, text, boolean, integer, integer) TO authenticated;
