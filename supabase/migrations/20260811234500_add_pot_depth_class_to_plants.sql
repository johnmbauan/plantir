-- Optional pot depth class for effective humidity interpretation on the dashboard / alerts.

ALTER TABLE public.plants
  ADD COLUMN IF NOT EXISTS pot_depth_class text NULL;

ALTER TABLE public.plants
  DROP CONSTRAINT IF EXISTS plants_pot_depth_class_check;

ALTER TABLE public.plants
  ADD CONSTRAINT plants_pot_depth_class_check CHECK (
    pot_depth_class IS NULL
    OR pot_depth_class IN (
      'compact',
      'small',
      'medium',
      'large',
...
COMMENT ON COLUMN public.plants.pot_depth_class IS
  'Optional abstract pot height class (compact/small/medium/large/deep/in_ground). Display cm bands are app-side only.';

-- Include pot_depth_class in the dashboard RPC payload.
...
BEGIN
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'pot_depth_class', p.pot_depth_class,
